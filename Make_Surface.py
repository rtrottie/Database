...
for atom in atoms:
    ...
    for (start, final) in runs:
        ...
        images = Poscar.from_dict(start['poscar']).structure.interpolate(Poscar.from_dict(final['poscar']).structure, nimages=6, autosort_tol=1)
        ...
        sd = [[True, True, True]] * len(Poscar.from_dict(start['poscar']).structure.sites)
        for i in [0, 4, 33, 36, 48]:
        ...
        for i in range(len(images)):
            ...
            Poscar(images[i], selective_dynamics=sd).write_file(os.path.join(folder, 'POSCAR'))
        os.chdir(run_folder)
        sub = subprocess.Popen(['/home/ryan/bin/nebavoid.pl', '1.45'])
        sub.wait()
